...
function showAddPersonDialog() {
    document.getElementById('add-person-dialog').show();
}

function hideAddPersonDialog() {
    document.getElementById('add-person-dialog').hide();
}

function addPerson() {
    const personName = document.getElementById('person-name').value;
    const personDistance = document.getElementById('person-distance').value;
...
function displayPersons() {
    ...
    personList.forEach(person => {
        ...
        listItem.innerHTML = `${person.name} - 距離: ${person.distance}`;
        list.appendChild(listItem);
    });
}

// スライダーの値をリアルタイムで更新
document.getElementById('person-distance').addEventListener('input', function() {
